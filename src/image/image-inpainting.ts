 ...
 */
export class ImageInpainting extends API {
  ...
  /**
   * Fetch Image Inpainting from the Nela AGI API based on the provided image, mask image, prompts and parameters.
   *
   * @param image - The image parameter is a required component of the request data format for the Image Inpainting API. It specifies the image file provided as a reference or starting point for generating the desired output image according to the provided prompts and parameters. image_content_type should be in PNG and JPEG image formats.
   * @param maskImage - The mask_image Parameter  is a required component of the request data format for the Image Inpainting API. It specifies the image file containing the mask that defines the region in the image where the restoration or filling should occur. Typically, this mask image is a binary image where the areas to be in-painted are marked as white (255) and the rest as black (0). The algorithm utilizes this mask to guide the inpainting process, ensuring that only the specified areas are affected by the restoration while preserving the rest of the image. mask_image_content_type should be in PNG and JPEG image formats.
   ...
   * @param width - The width parameter, set to 1024 by default, specifies the desired width of the generated image in pixels. It controls the horizontal dimension of the output image, influencing its aspect ratio and overall size. Width should be between 512 and 1024 px.
   * @param height - The height parameter, set to 1024 by default, specifies the desired height of the generated image in pixels. Similar to the width parameter, this controls the vertical dimension of the output image, allowing customization of its size and aspect ratio. Height should be between 512 and 1024 px.
   ...
   * @param numInferenceSteps - The num_inference_steps parameter, set to 25 by default, specifies the number of inference steps or iterations the AI model should perform during the image inpainting process. Increasing this parameter can enhance the complexity and detail of the generated image but may also prolong the processing time. num_inference_steps should be between 1 and 75 value.
   ...
   *
   * @returns {Promise<AxiosResponse>} - A Promise that resolves with the result of the image inpainting process in the following format: 
   * @example
  * ```json
   * {
      "data": {
